...
| 02 | 자동차 | **움직입니다** — 차선이 흐르고 바퀴가 돌고 차체가 튄다. 위치 = 사용률 | 승용차 / 화물차 |
| 03 | 배터리 | 잔량 = 남은 한도 | 두 개 |
...
**움직이는 테마**는 `Anim = 150`(프레임 간격 ms)을 선언하면 그 주기로 다시 그려지고 `$d.F` 가 프레임 번호로 들어옵니다. 숨겨져 있을 땐 자동으로 멈춥니다.
⚠️ 매 프레임 벡터를 다시 그리면 비쌉니다 — `Fill-RoundRect` 는 호출마다 GraphicsPath 를 만들어 자동차 테마가 6.8ms/frame, CPU 8% 였습니다.
정적 배경과 움직이는 조각을 **미리 비트맵으로 구워 두고 붙이면** 1.2ms/frame 까지 내려갑니다(`02-cars.ps1` 참고).
그래도 창 재합성 비용 때문에 자동차 테마는 **CPU 약 3%** 를 씁니다(다른 테마는 약 1%) — 배터리가 아쉬우면 정적 테마를 쓰세요.
